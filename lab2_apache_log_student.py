...
dayGroupedHosts = dayToHostPairTuple.groupByKey().map(lambda dh: (dh[0], set(dh[1])))

dayHostCount = dayGroupedHosts.mapValues(len).sortByKey()
...
daysWithHosts = dailyHosts.map(lambda (day, hosts_count) : day).collect()
hosts = dailyHosts.map(lambda (day, hosts_count): hosts_count).collect()
...
avgDailyReqPerHost = (sortedByDay
                      .map(lambda ( day,(data)) : (day, data[0]/data[1]))
                      .cache())
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, key=lambda x: -1 * x[1])
...
errDateSum = errDateCountPairTuple.reduceByKey(lambda a, c: a + c)
...
hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a,c : a+c)
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (hour, error) : hour).collect()
errors404ByHours = hourRecordsSorted.map(lambda (hour, error) : error).collect()
